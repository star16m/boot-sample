    ...
    protected void registerHandlerMethod(Object handler, Method method, RequestMappingInfo mapping) {
        ...
        if (SimpleUtil.isNotNull(methodMapping)) {
            ...
            if (SimpleUtil.isNotNull(restController)) {
                Optional<SimpleRestMethod> supportMethods = Arrays.asList(restController.supportedMethod()).stream()
                        .filter(supportMethod -> supportMethod.equals(methodMapping.value()))
                        .findAny();
